...
class CartStorage extends Iterable
{
	...
	{
		ProductGroup<T> matchedGroup = _cart.firstWhere(
				(group)=> group == group2add,
				orElse:(){
			_cart.add(group2add);
			return group2add;
		});
	...
	{
		ProductGroup<T> matchedGroup = _cart.firstWhere(
				(group)=> group == product, orElse:(){
		var newGroup = new ProductGroup<T>(product);
			_cart.add(newGroup);
			return newGroup;
		});
		matchedGroup.addProduct();
	...
	{
		ProductGroup<T> matchedGroup = _cart.firstWhere(
				(group)=> group==group2remove, orElse:()=>null);
	...
	{
		ProductGroup<T> matchedGroup = _cart.firstWhere(
				(group)=> group==product, orElse:()=>null);
		if (matchedGroup == null) return;
		matchedGroup.removeProduct();
		if (matchedGroup.count == 0)
		{
