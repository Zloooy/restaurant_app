...
class HexagonalButton extends StatelessWidget
{
	...
	HexagonalButton({@required this.child, this.diameter = double.infinity, this.borderRadius = const Radius.circular(0), @required this.onPressed});
...
class HexagonalBorder extends ShapeBorder
{
	...
	Path getOuterPath(Rect rect, {TextDirection textDirection})
	{
		...
		Path res = new Path()
				..moveTo(rect.left + rect.width/2 + sideVector.length/4, rect.top + borderRadius.y.abs());
		/*pos
		...
		//print("start pos:${pos.x} ${pos.y}");
			...
			for (int i = 0; i<6; i++)
		{
			Vector2 hsClone = halfSide.clone()..normalize()..postmultiply(halfRotationMatrix);
			hsClone *= sqrt(pow(borderRadius.x.abs(),2)  + pow(borderRadius.y.abs(),2));
			//print("half side $halfSide");
			halfSide.postmultiply(rotationMatrix);
			res.relativeQuadraticBezierTo(hsClone.x, hsClone.y, halfSide.x, halfSide.y);
